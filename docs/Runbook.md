...
- `npm run search:reindex` — полностью пересобрать Typesense-индекс из текущей БД.
- `npm run search:worker` — один раз обработать очередь search_queue.
- `npm run search:watch` — держать воркер очереди запущенным в watch-режиме.
...
`navbar`-поиск пустой или результаты заметно неполные
- Если Typesense не поднят или индекс не собран, `/api/search` переключается на fallback через PostgreSQL.
- Проверить `TYPESENSE_*` в `.env`, затем выполнить `npm run search:reindex`.
- Если нужен именно полнотекстовый multi-search с актуальными группами и ранжированием, убедиться, что воркер очереди поиска тоже отрабатывает без ошибок.
